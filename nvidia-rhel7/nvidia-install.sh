...
#
# Automated NVIDIA re-install script for EL7
#

# The four vars below need to be updated
# with your directory and install pkg filenames
INSTALLER_DIR='/mnt/somewhere'
DRIVER_PKG_NEW='placeholder-x86_64.run'
DRIVER_PKG_340='placeholder-340-x86_64.run'
DRIVER_PKG_304='placeholder-304-x86_64.run'

# No user modifications after this point
...
if [[ -z $(lspci -nd "0x10de:*:*") ]]; then
  NOTNVIDIA=true
  exit 0
fi

## See if we already have a built module running
if [[ -e /lib/modules/$(uname -r)/kernel/drivers/video/nvidia.ko ]]; then
  MODULEOK=true
  echo "NVIDIA module matches running kernel"
  exit 0
fi

## Terrible for-loops to determine which
## type of NVIDIA drivers we need
for i in ${LEGACY340ID};
do
  if [[ ! -z $(lspci -nd "0x10de:${i}:0x0300") ]]; then 
    LEGACY340=true
    DRIVER_PKG=${DRIVER_PKG_340}
    echo "Legacy 340 Device ${i}"
  fi
done

for i in ${LEGACY304ID};
do
  if [[ ! -z $(lspci -nd "0x10de:${i}:0x0300") ]];then 
    LEGACY304=true
    DRIVER_PKG=${DRIVER_PKG_304}
    echo "Legacy 304 Device ${i}"
  fi
done

if [[ ${LEGACY340} == false ]] && [[ ${LEGACY304} == false ]] && [[ ${NOTNVIDIA} == false ]]; then
  LATESTNV=true
  DRIVER_PKG=${DRIVER_PKG_NEW}
  echo "Latest NVIDIA Device ${i}"
fi

## Install drivers
if [[ ${LEGACY340} == true ]] || [[ ${LEGACY304} == true ]] || [[ ${LATESTNV} == true ]]; then
  if [ -e "${INSTALLER_DIR}${DRIVER_PKG}" ]; then
    echo "Installing ${INSTALLER_DIR}${DRIVER_PKG}"
    ${INSTALLER_DIR}${DRIVER_PKG} -qasX --tmpdir /root
  else
    echo "Cannot reach mountpoint"
    exit 1
  fi
else
